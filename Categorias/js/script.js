...
function sortByRelevance(results, term) {
  ...
  if (menuHamburguer && navgation) {
   ...
   const artistasLinhas = document.querySelectorAll('.artistas-linha');
  
  artistasLinhas.forEach(linha => {
    let isDown = false;
    let startX;
    let scrollLeft;
    ...
    linha.addEventListener('mousedown', (e) => {
      isDown = true;
      startX = e.pageX - linha.offsetLeft;
      scrollLeft = linha.scrollLeft;
    });
    ...
    linha.addEventListener('mouseup', () => {
      isDown = false;
    });
    ...
    linha.addEventListener('mousemove', (e) => {
      if(!isDown) return;
      e.preventDefault();
      const x = e.pageX - linha.offsetLeft;
      const walk = (x - startX) * 2;
      linha.scrollLeft = scrollLeft - walk;
    });

    // Suporte para touch devices
    linha.addEventListener('touchstart', (e) => {
      isDown = true;
      startX = e.touches[0].pageX - linha.offsetLeft;
      scrollLeft = linha.scrollLeft;
    });

    linha.addEventListener('touchend', () => {
      isDown = false;
    });

    linha.addEventListener('touchmove', (e) => {
      if(!isDown) return;
      e.preventDefault();
      const x = e.touches[0].pageX - linha.offsetLeft;
      const walk = (x - startX) * 2;
      linha.scrollLeft = scrollLeft - walk;
    });
  });
